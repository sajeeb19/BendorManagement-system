...
if(isset($_GET)){
    ...
    $spl1= "SELECT * FROM work_order WHERE agentId= $id " ;
    $result1 = mysqli_query($conn,$spl1) or die( mysqli_error($conn));
    $employinfo1 = mysqli_fetch_all($result1,MYSQLI_ASSOC);
    

    foreach( $employinfo1 as $new){
       $newID=$new['agentId'];
       $sql2 = "SELECT firstName,lastName  FROM employ_information WHERE employId = $newID";
       $result2 = mysqli_query($conn,$sql2) or die( mysqli_error($conn));
       $agentName = mysqli_fetch_array($result2);
    //    print_r($agentName);
                                 ...
                                 <?php foreach ($employinfo1 as $item) : ?>
                                  ...
                                  <div class="row z-depth-1 workorderRow">
                                        ...
                                        <div class="col s2  center"> 
                                            <a href="report.php?id=<?php echo $employinfo['employId']?> & wonumber=<?php echo urldecode($item['workOrderId'])  ?> " class=" "><i class=" medium material-icons  hoverable  foricon "> credit_card </i></a>
                                            <!-- <a class="btn pulse"><i class="material-icons foricon">menu</i></a> -->
                                    
                                         </div>
                                        <div class="col s2 center">
                                            <a href="workOrderView.php?id=<?php echo $employinfo['employId']?> & wonumber=<?php echo urldecode($item['workOrderId'])  ?> " class=" "><i class=" medium material-icons  hoverable  foricon "> view_module</i></a>
